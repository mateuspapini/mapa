(() => {
  ...
  if (modal && modalBox) {
    ...
    document.querySelectorAll(".pf-card[data-id]").forEach((card) => card.addEventListener("click", () => {
      opener = card;
    }));
    const syncModal = () => {
  ...
  const updateTabs = () => document.querySelectorAll(".srv-tab").forEach((tab, i) => {
    tab.id = "srv-tab-" + i;
    tab.setAttribute("aria-controls", "srvPanel");
    tab.tabIndex = tab.getAttribute("aria-selected") === "true" ? 0 : -1;
  });
  new MutationObserver(updateTabs).observe(tabs, { childList: true });
  updateTabs();
  if (reduced.matches) document.querySelectorAll("[data-count]").forEach((el) => {
