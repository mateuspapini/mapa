(function() {
  ...
  var ADS_CONVERSOES = {
    whatsapp: "1jU0CMWU1YAdEKDf0eVE",
    email: "mghRCMiU1YAdEKDf0eVE"
  };
  ...
  function lerConsentimento() {
    try {
      ...
      var dias = (Date.now() - o.data) / 864e5;
  ...
  function gravarConsentimento(estado) {
    try {
      localStorage.setItem(CONSENT_KEY, JSON.stringify({ estado, data: Date.now() }));
    } catch (e) {
    }
  }
  ...
  function evento(nome, params) {
    ...
    p.idioma = idioma();
    try {
      if (typeof window.gtag === "function") window.gtag("event", nome, p);
    } catch (e) {
    }
    try {
      window.dataLayer.push(Object.assign({ event: nome }, p));
    } catch (e) {
    }
  }
  ...
  function conversao(chave, params) {
    ...
    p.send_to = destino;
    try {
      if (typeof window.gtag === "function") window.gtag("event", "conversion", p);
    } catch (e) {
    }
  }
  var TXT = {
  ...
  function txt() {
    return TXT[idioma()] || TXT.pt;
  }
  var CSS = [
    ".mapa-cc{position:fixed;left:1rem;right:1rem;bottom:1rem;z-index:9998;",
    "max-width:640px;margin:0 auto;background:var(--card,#1a1e25);",
    "border:1px solid var(--bdr,rgba(255,255,255,.1));border-radius:6px;",
    "padding:1.05rem 1.15rem;box-shadow:0 18px 40px -20px rgba(0,0,0,.5);",
    ...
    ".mapa-cc p{font-size:.84rem;line-height:1.65;color:var(--t2,#a4acb8);margin:0 0 .85rem}",
    ...
    "border-radius:6px;padding:.6rem 1.15rem;border:1px solid transparent;transition:background-color .2s,border-color .2s,color .2s}",
    ".mapa-cc .mapa-cc-ok{background:#2f6ad9;color:#fff;flex:1 1 auto}",
    ".mapa-cc .mapa-cc-no{background:transparent;color:var(--t2,#a4acb8);border-color:var(--bdr,rgba(255,255,255,.14));flex:1 1 auto}",
    ".mapa-cc .mapa-cc-ok:hover{background:#2759b8}",
    ".mapa-cc .mapa-cc-no:hover{border-color:var(--t3,#8c95a2);color:var(--t1,#eceef2)}",
    ".mapa-cc button:focus-visible{outline:2px solid var(--focus,#7aa7f7);outline-offset:2px}",
    ...
    ".mapa-cc p a{color:var(--accent-txt,#7aa7f7);text-decoration:underline;text-underline-offset:3px}",
    ".mapa-cc-p{text-align:center;margin-top:.5rem;display:flex;gap:.5rem;justify-content:center;flex-wrap:wrap}",
    ".mapa-cc-p a{font-size:.76rem;color:var(--t3,#8c95a2);text-decoration:underline;text-underline-offset:3px}",
    ".mapa-cc-p a:hover{color:var(--accent-txt,#7aa7f7)}",
    ".mapa-cc-p .sep{font-size:.76rem;color:var(--t3,#8c95a2);opacity:.5}",
    ".mapa-cc-p button{background:none;border:0;padding:0;cursor:pointer;font:inherit;",
    "font-size:.76rem;color:var(--t3,#8c95a2);text-decoration:underline;text-underline-offset:3px}",
    ".mapa-cc-p button:hover{color:var(--accent-txt,#7aa7f7)}"
  ].join("");
  function injetarCSS() {
  ...
  function atualizarTextos() {
    ...
    if (linkTermos) linkTermos.textContent = d.termos;
  }
  ...
  function linkRodape() {
    ...
    var destino = alvo.querySelector(".ft-bottom") || alvo.querySelector(".ft-btm") || alvo.querySelector(".wrap") || alvo;
    var d = txt();
    var p = document.createElement("p");
    ...
    t.href = "/termos-de-uso/";
    t.textContent = d.termos;
      ...
      if (href.indexOf("wa.me") !== -1 || href.indexOf("api.whatsapp.com") !== -1) {
        ...
        try {
          if (typeof window.clarity === "function") {
            ...
            window.clarity("upgrade", "contato");
          }
        } catch (e) {
        }
        return;
  ...
  (function() {
    ...
    form.addEventListener("input", function() {
      ...
      try {
        if (typeof window.clarity === "function") window.clarity("set", "usou_calculadora", "sim");
      } catch (e) {
      }
    });
  })();
