(function(){
  ...
  const io = new IntersectionObserver(entries => {
    entries.forEach(e => {
      if (e.isIntersecting) {
        // aria-current + estilo via CSS: o estado ativo acompanha o tema
        // e não é sinalizado apenas por cor (WCAG 1.4.1 / 1.4.3).
  ...
  (function() {
    ...
    /* Rótulo anuncia a AÇÃO do botão, não um estado genérico */
    const LABEL = {
    ...
    function applyTheme(isDark) {
      ...
      if (meta) meta.setAttribute('content', isDark ? '#080c18' : '#f2f5fc');
      labelTheme();
